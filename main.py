...
async def webhook(token):
    ...
    embed.add_embed_field(name='Binance', value=await binance(token), inline=True)
    embed.add_embed_field(name='Coinlist', value=await coinlist(token), inline=True)
    embed.add_embed_field(name='Coinbase', value=await coinbase(token), inline=True)
    embed.add_embed_field(name='FTX', value=str(await ftx(token)), inline=True)
...
bot.run('ODMzMDg1OTEzMTkzNzc1MTI0.YHtNtw.o9YebQKP5roI1y3FEImuV0erDfc')